<%-- 
    ...
    Created on : Mar 23, 2022, 12:43:18 AM
    Author     : Yu Yang
--%>
...
<html lang="en">
    ...
    <body onload="chkVoucherStatus('<%=session.getAttribute("ssTicVoucherValidation")%>')">
        <div class="navbar">
    ...
    <div class="movie-table-container" style="margin-right:200px;">
        ...
        <div class="container">
            <div class="custom-content">
                <h1 class="pageTitle">Transaction Summary</h1> <hr/>
                <br/>
                <%--Form that display the transaction info to the user and allow them to press the proceed btn after confirm with the stated details --%>
                <form id="transactionSumForm" action="ValidateVoucherServlet" method="post">
                    <table id="transactionSumTable">
                        <colgroup>
                            <col class="desc" style="width: 60%;"/>
                            <col class="value" style="width: 40%;"/>
                        </colgroup>

                        <%--Here will be using the EL language to display the respective details from the JavaBean --%>
                        <tr>
                            <td>Movie Title: </td>
                            <td>${ssTicSelectedSchedule.getMovie().getMovieTitle()}</td>
                        </tr>
                        <tr>
                            <td>Showing Date: </td>
                            <td>${ssTicSelectedSchedule.getSchedule_date()}</td>
                        </tr>
                        <tr>
                            <td>Showing Time: </td>
                            <td>${ssTicSelectedSchedule.getSchedule_time()}</td>
                        </tr>
                        <tr>
                            <td>Hall No: </td>
                            <td>${ssTicSelectedSchedule.getHall().getHall_no()}</td>
                        </tr>
                        <tr>
                            <td>Seat Selected: </td>
                            <td>${selectedSeat}</td>
                        ...
                        <tr>
                            ...
                            <td>${ssTicPayment.adult_qty}</td>
                        </tr>
                        <tr>
                            <td>Total Child Ticket(s) Purchased: </td>
                            <td>${ssTicPayment.child_qty}</td>
                        </tr>
                        <tr>
                            <td>Total Price: </td>
                            <td><%=String.format("RM%.2f", ssTicPayment.getAmount()) %></td>
                        </tr>
                        <tr>
                            <%-- Place that allow user to enter the valid voucher code --%>
                            <td>Enter Voucher Code Here (IF ANY): </td>
                            <td><input type="text" name="voucherCode" maxlength = "7" /></td>
                        </tr>
                    </table>
                    <br/>
                    <input type="submit" value="Proceed" class="proceedBtn" style="margin: 0 47%"/>
                </form>
            </div>
        </div>
  	 ...
  	 <div class="footer-container">
        ...
        <script>
            function chkVoucherStatus(status){ //function to print the errMsg to user if the voucher code entered is invalid
                if(status === "Invalid"){
                    alert('Voucher Code entered is invalid. Please reenter!');
                }
            }
        </script>
    </body>
