...
<jsp:useBean id="ssCurrentMovie" class="model.Movie" scope="request"/>
<%
    ...
    <div class="container">
        <div class="custom-content2">
            <form action="EditMovie" method="post" enctype="multipart/form-data">
                <div class="form">
                    <div class="title">
                        <h1 style="font-size: 50px;padding: 20px 0 20px 50px;color:#ffe229;">Edit Movie</h1>
                        <hr style="margin-left:20px" width="1270px">
                    </div>
                    <div class="posterField">
                        <label class="inputPoster">Poster:</label>
                        <div class="image-preview" id="imagePreview">
                            <img style="display:block" alt="Image Preview" class="poster" src="<%=ssCurrentMovie.getImgString()%>" id="poster"/>
                            <span class="image-preview-text" style="display:none">Movie Poster<br/>(27 x 40) inches</span>
                        </div>  
                        <br/>
                        <input type="file"  name="poster" accept="image/jpg, image/jpeg, image/png*" id="input-image">
                    </div>
                    <div class="input-wrapper"  style="grid-row-start:2">
                       <label style="margin-left:30px;" ><b>Movie ID:</label>
                       <input type="text" class="ID" name="movieID" value="<%=ssCurrentMovie.getMovieID()%>" readonly="readonly"></text>
                       </b>
                    </div>
                    <div class="input-wrapper"  style="grid-row-start:3">
                        <label>Movie Title:</label>
                        <br/>
                        <input type="text" class="input-field" name="movieTitle" value="<%=ssCurrentMovie.getMovieTitle()%>" id="movieTitle" required>
                    </div>
                    <div class="input-wrapper"  style="grid-row-start:3">
                        <label>Duration:</label>
                        <br/>
                        <input type="number" min="0" max="500" class="input-field" name="duration" value="<%=ssCurrentMovie.getDuration()%>" id="duration" required>
                    </div>
                    <div class="input-wrapper"  style="grid-row-start:4">
                        <label>Age Rating:</label>
                        <br/>
                        <select style="width:310px;height:35px" name="ageRating" id="ageRating" required>
                            <option>U</option>
                            <option>P13</option>
                            <option>18</option>
                            <option>TBC</option>
                    ...
                    <div class="input-wrapper"  style="grid-row-start:4">
                        ...
                        <br/>
                        <input type="date" id="datefield" class="input-field" name="releaseDate" value="<%=ssCurrentMovie.getReleaseDate()%>" required>
                    </div>
                    <div class="input-wrapper"  style="grid-row-start:5">
                        <label>Director:</label>
                        <br/>
                        <input type="text" class="input-field" name="director" value="<%=ssCurrentMovie.getDirector()%>" id="director" required>
                    </div>
                    <div class="input-wrapper"  style="grid-row-start:5">
                        <label>Distributor:</label>
                        <br/>
                        <input type="text" class="input-field" name="distributor" value="<%=ssCurrentMovie.getDistributor()%>" id="distributor" required>
                    </div>
                    <div class="input-cast-wrapper">
                        <label style="margin: 0 0 0 40px;display:block;padding-top:10px">Cast:</label>
                        <% 
                            String cast="";
                            ArrayList<String> casts = (ArrayList)request.getAttribute("ssCurrentCast");
                            for(int j=0;j<casts.size();j++){
                                cast = cast + casts.get(j);
                                if(j!=casts.size()-1){
                                    cast=cast+",";
                                }    
                            }
                        %> 
                        <input type="text" class="input-field" style="width:670px;margin-left:35px;margin-bottom: 20px" name="cast" id="selected-cast" value="<%=cast%>" required>
                        <br/>
                    ...
                    <div class="input-wrapper"  style="grid-row-start:7; grid-column-start: span 2;">
                        ...
                        <br/>
                        <textarea name="synopsis" id="textarea" onkeyup="getWordCount()" class="input-field" style="width:750px;height:150px" maxlength="500" required><%=ssCurrentMovie.getSynopsis()%></textarea>
                        <br/>
                        <text id="word-count" class="word-count">0/500</text>
                    </div>
                    <div style="grid-row-start:8;grid-column-start:span 3;">
                        <button type="submit" class="crud" style="font-size:20px; width:150px;margin:0 150px 0 0;">Edit</button>
