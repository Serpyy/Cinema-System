...
<html>
    ...
    <body>
        ...
        <h1 style="color: yellow;text-align: center" >SALES REPORT</h1>
 <input  class="refundsearch" type="text" id="myInput" onkeyup="myFunction()" placeholder="Search for names.." title="Type in a name">
<div class="Scroll">
    
        <table class="saleReporttable" border="1" id="SaleTable">
            <th onclick="sortTable(0)">Customer</th>
            <th onclick="sortTable(1)">Movie</th>
            <th onclick="sortTable(2)">Purchase Date</th>
            <th onclick="sortTable(3)">Schedule Date</th>
            <th onclick="sortTable(4)">Schedule Time</th>
            <th onclick="sortTable(5)">Adult Quantity</th>
            <th onclick="sortTable(6)">Child Quantity</th>
            <th onclick="sortTable(7)">Price</th>
       

        <%
            Payment[] paymentlist = (Payment[]) session.getAttribute("PaymentList");
            Ticket[] ticketList = (Ticket[]) session.getAttribute("TicketList");
             int adult = (Integer) session.getAttribute("adult");
              int child = (Integer) session.getAttribute("child");
               double total = (Double) session.getAttribute("total");
            for (int i = 0; i < paymentlist.length; i++) {

        %>
        <tr class="trsalereport">
            <td><%= paymentlist[i].getCard().getCardholder_name() %></td>
            <td><%= ticketList[i].getSchedule().getMovie().getMovieTitle() %></td>
            <td><%= paymentlist[i].getPayment_date() %></td>
            <td><%= ticketList[i].getSchedule().getSchedule_date() %> </td>
            <td><%= ticketList[i].getSchedule().getSchedule_time() %> </td>
            <td><%= paymentlist[i].getAdult_qty() %> </td>
            <td><%= paymentlist[i].getChild_qty() %> </td>
            <td><%= paymentlist[i].getAmount() %></td>
        </tr>
         
        <% }%>
        <th style="bottom: 0px;position: sticky;height: 70px"></th>
        <th style="bottom: 0px;position: sticky"></th>
        <th style="bottom: 0px;position: sticky"></th>
        <th style="bottom: 0px;position: sticky"></th>
        <th style="bottom: 0px;position: sticky"></th>
        <th style="bottom: 0px;position: sticky">Total  = <%= adult %></th>
        <th style="bottom: 0px;position: sticky">Total  = <%= child %></th>
        <th style="bottom: 0px;position: sticky">Total  =   <%= total %></th>
