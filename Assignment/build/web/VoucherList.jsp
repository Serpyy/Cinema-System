...
<%
    ...
    ArrayList<Voucher> voucherList = (ArrayList<Voucher>)session.getAttribute("ssVoucherList");
...
<html lang="en">
  ...
  <body>
    ...
    <div class="container">
      <h1 id="voucherh1"> Membership Voucher</h1>
      <br />
        
      <br />

      <div id="cust_vocher">
        <table id="custVoucher_table">
          <thead>
            <th>Voucher No</th>
            <th>Discount ( RM )</th>
          </thead>
          <tbody>
              <%
                ...
                %>
                <tr>
                    <td colspan="2" class="norecord" >Sorry, you do not have any voucher.</td>
                <tr>
                <%
                    }else{  
                ...
                %>
               <tr> 
                   <td><%= voucher.getVoucher_no() %></td>
                   <td><%= voucher.getDis_amount() %></td>
               </tr>
              <%}}%>             
          </tbody>
        </table>
      ...
      <div id="redeem_vocher" style="--s: 'Point : <%=cust.getPoint()%>'" >
            <table id="voucher_table">
                ...
                <tr>
                <form action="Voucher" method="post" onsubmit="return confirm('Are you sure you want to redeem a RM 5 vocuher?');" >
                    <td><input type="number" class="dis_value" value="5" readonly="true" name="txtdis" size="5"/></td></td>
                    <td> 50 point</td>
                    <%  if(cust.getPoint() >= 50 ){  %>
                            <td><input type="submit" class="redeembtn" value="Redeem"  onclick=""  /></td>
                    <%}else{%>
                             <td><input type="submit" class="redeembtn" value="Redeem" disabled  / ></td>                    
                    <%}%>
                </form>
                </tr>
                <tr>
                <form action="Voucher" method="post" onsubmit="return confirm('Are you sure you want to redeem a RM 10 Vocuher?');" >
                    <td><input type="number"  class="dis_value" value="10" readonly="true" name="txtdis" size="5"/></td></td>
                        <td> 100 point</td>
                        <%  if(cust.getPoint() >= 100 ){  %>
                            <td><input type="submit" class="redeembtn" value="Redeem"  /></td>
                        <%}else{%>
                             <td><input type="submit" class="redeembtn" value="Redeem" disabled  /></td>                    
                        <%}%>
                </form>
                ...
                <tr>
                   <form action="Voucher" method="post" onsubmit="return confirm('Are you sure you want to redeem a RM 15 Vocuher?');" >
                       <td><input type="number" class="dis_value" value="15" readonly="true" name="txtdis" size="5"/></td>
                        <td>150 point</td>
                        <%  if(cust.getPoint() >= 150 ){  %>
                            <td><input type="submit" class="redeembtn" value="Redeem" /></td>
                        <%}else{%>
                             <td><input type="submit" class="redeembtn" value="Redeem" disabled  /></td>                    
                        <%}%>
